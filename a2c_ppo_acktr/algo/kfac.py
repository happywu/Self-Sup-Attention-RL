...
class KFACOptimizer(optim.Optimizer):
    def __init__(self,
        ...
        def split_bias(module):
            # recursively find the last module, then split bias
            for mname, child in module.named_children():
                if '_feat_encoder' in mname: continue
                if '_keypointer' in mname: continue
                if 'selfsup_attention' in mname: continue
                if hasattr(child, 'bias') and child.bias is not None:
                    module._modules[mname] = SplitBias(child, mname)
                else:
                    split_bias(child)
    ...
    def _prepare_model(self):
        for module in self.model.modules():
            ...
            if classname in self.known_modules:
                ...
                if not ((classname in ['Linear', 'Conv2d']) and module.bias is not None):
                    has_bias = True
                else:
                    has_bias = False

                if not hasattr(module, 'split_bias'): continue
    ...
    def step(self):
        ...
        for i, m in enumerate(self.modules):
            assert len(list(m.parameters())
                       ) == 1, "Can handle only one parameter at the moment, has " \
                               "{}".format(len(list(m.parameters())))
