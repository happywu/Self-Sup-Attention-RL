...
def make_env(env_id, seed, rank, log_dir, allow_early_resets, width=84, height=84, ram_wrapper=False):
    def _thunk():
        ...
        if is_atari:
            if len(env.observation_space.shape) == 3:
                env = wrap_deepmind(env, width=width, height=height)
...
def make_vec_envs(env_name,
                  ...
                  ram_wrapper=False):
    envs = [
        make_env(env_name, seed, i, log_dir, allow_early_resets, width=width, height=height, ram_wrapper=ram_wrapper)
